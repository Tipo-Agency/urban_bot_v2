...
async def freeze_confirmed_handler(callback: CallbackQuery):
    ...
    if freeze_response:
        if freeze_response.get("result"):
            await callback.message.edit_text("✅ Подписка успешно заморожена.", reply_markup=get_cabinet_keyboard(ticket_id=ticket_id, is_subscriped=True, is_freezed=True))
    else:
        await callback.message.edit_text("❌ Не удалось заморозить подписку. Ваш лимит на заморозку исчерпан", reply_markup=get_cabinet_keyboard(ticket_id=ticket_id, is_subscriped=True, is_freezed=False))
    await callback.answer()
